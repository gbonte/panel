...
## Experimental comparison on volatility data of the following forecasting algorithms
...
for (nn in c(0:6)){ ## volatility index
  allS=NULL
  allE=NULL
  allE2=NULL
  allE3=NULL
  ...
  for (H in c(2,5,10,20,50)){ ## horizons
    set.seed(nn+H)
    namefile=paste("./data/bench.vola",nn,"Rdata",sep=".")
    ...
    if (is.element(2,Algos)){
      cat("D: pcadesign \n")
      ptm <- proc.time()
      P=pcadesign(Xtr,2*m,H,2*p0,CC=2*CC,lambda=0)
      print(P$mod)
    ...
    for (s in seq(Ntr,N-H,length.out=200)){
...
######## ALGO 11
      ...
      allE=rbind(allE,c(H,nn,mean(e.hat)))
      allE2=rbind(allE2,c(H,nn,mean(e.hat2)))
      allE3=rbind(allE3,c(H,nn,mean(e.hat3)))
      allE4=rbind(allE4,c(H,nn,mean(e.hat4)))
      allE5=rbind(allE5,c(H,nn,mean(e.hat5)))
      allE6=rbind(allE6,c(H,nn,mean(e.hat6)))
      allE7=rbind(allE7,c(H,nn,mean(e.hat7)))
      allE8=rbind(allE8,c(H,nn,mean(e.hat8)))
      allE9=rbind(allE9,c(H,nn,mean(e.hat9)))
      allE10=rbind(allE10,c(H,nn,mean(e.hat10)))
      allE11=rbind(allE11,c(H,nn,mean(e.hat11)))
      allE0=rbind(allE0,c(H,nn,mean(e.hat0)))
    ...
    ##### PRINT OUT OF RESULTS
          ...
          indH<-which(allE0[,1]==H & allE0[,2]==nn)
          cat("\n **** \n nn=",nn," H=",H)
          cat( "Naive :",mean(allE0[indH,3]))
          if (is.element(1,Algos))
                    cat( namesAlgos[1],":",mean(allE[indH,3]))
          if (is.element(2,Algos))
                    cat(namesAlgos[2],":",mean(allE2[indH,3]))
          if (is.element(3,Algos))
                    cat(namesAlgos[3],":",mean(allE3[indH,3]))
          if (is.element(4,Algos))
                    cat(namesAlgos[4],":",mean(allE4[indH,3]))
          if (is.element(5,Algos))
                    cat(namesAlgos[5],":",mean(allE5[indH,3]))
          if (is.element(6,Algos))
                    cat(namesAlgos[6],":",mean(allE6[indH,3]))
          if (is.element(7,Algos))
                    cat(namesAlgos[7],":",mean(allE7[indH,3]))
          if (is.element(8,Algos))
                    cat(namesAlgos[8],":",mean(allE8[indH,3]))
          if (is.element(9,Algos))
                    cat(namesAlgos[9],":",mean(allE9[indH,3]))
          if (is.element(10,Algos))
                    cat(namesAlgos[10],":",mean(allE10[indH,3]))
           if (is.element(11,Algos))
                      cat(namesAlgos[11],":",mean(allE11[indH,3]))

    
   ##### SAVING RESULTS    
    
    namefile=paste("./results/volabench",nn,"Rdata",sep=".")
    if (savefile)
      save(file=namefile,
